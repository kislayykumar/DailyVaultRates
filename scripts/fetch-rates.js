...
// ─────────────────────────────────────────────────────────────────────────────
// GOODRETURNS  —  Indian retail INR prices for Gold & Silver (Delhi)
// ─────────────────────────────────────────────────────────────────────────────


async function scrapeGoodReturnsGold() {
 try {
   const html = await fetchText('https://www.goodreturns.in/gold-rates/delhi.html');


   // Primary: intro paragraph — "&#x20b9;15,212 per gram for 24 karat ... &#x20b9;13,945 per gram for 22 karat ... &#x20b9;11,413 per gram for 18"
   const introRegex = /&#x20b9;([\d,]+)\s*<\/strong>\s*per gram for 24 karat[\s\S]*?&#x20b9;([\d,]+)\s*<\/strong>\s*per gram for 22 karat[\s\S]*?&#x20b9;([\d,]+)\s*<\/strong>\s*per gram for 18/i;
   const introMatch = html.match(introRegex);
   if (introMatch) {
     const g24k = parseFloat(introMatch[1].replace(/,/g, ''));
     const g22k = parseFloat(introMatch[2].replace(/,/g, ''));
     const g18k = parseFloat(introMatch[3].replace(/,/g, ''));
     console.log(`  [GoodReturns-Delhi] Gold 24K=₹${g24k}/g  22K=₹${g22k}/g  18K=₹${g18k}/g`);
     return { g24k, g22k, g18k };
   }


   // Fallback: first "1 gram" data row in table
   const tableRegex = /<td>\s*1\s*<\/td>\s*<td>\s*(?:&#x20b9;|₹)?\s*([\d,]+)\s*<\/td>\s*<td>\s*(?:&#x20b9;|₹)?\s*([\d,]+)\s*<\/td>\s*<td>\s*(?:&#x20b9;|₹)?\s*([\d,]+)\s*<\/td>/i;
   const tableMatch = html.match(tableRegex);
   if (tableMatch) {
     const g24k = parseFloat(tableMatch[1].replace(/,/g, ''));
     const g22k = parseFloat(tableMatch[2].replace(/,/g, ''));
     const g18k = parseFloat(tableMatch[3].replace(/,/g, ''));
     console.log(`  [GoodReturns-Delhi] Gold 24K=₹${g24k}/g  22K=₹${g22k}/g  18K=₹${g18k}/g (table)`);
     return { g24k, g22k, g18k };
   }


   throw new Error('No regex matched gold prices');
 } catch (err) {
...
async function scrapeGoodReturnsSilver() {
 try {
   const html = await fetchText('https://www.goodreturns.in/silver-rates/delhi.html');


   // Primary: intro paragraph — "&#x20b9;245 per gram"
   const introRegex = /&#x20b9;([\d,]+)\s*<\/strong>\s*per gram/i;
   const introMatch = html.match(introRegex);
   if (introMatch) {
     const gramPrice = parseFloat(introMatch[1].replace(/,/g, ''));
     console.log(`  [GoodReturns-Delhi] Silver ₹${gramPrice}/g`);
     return { gramPrice, kgPrice: gramPrice * 1000 };
   }


   // Fallback 1: "₹ 2,45,000/kg" style
   const regexKg = /&#x20b9;\s*([\d,]+)\s*\/\s*kg/i;
   ...
   if (matchKg) {
     ...
     console.log(`  [GoodReturns-Delhi] Silver ₹${gramPrice}/g (from /kg)`);
     return { gramPrice, kgPrice };
   }


   // Fallback 2: table "1 gram" row
   ...
   if (matchRow) {
     ...
     console.log(`  [GoodReturns-Delhi] Silver ₹${gramPrice}/g (table row)`);
     return { gramPrice, kgPrice: gramPrice * 1000 };
   }


   throw new Error('No regex matched silver prices');
 } catch (err) {
...
async function main() {
 ...
 const PRICE_SANITY = {
   'gold-24k':  { min: 3000,   max: 20000,    field: 'priceUsdOunce' },
   'gold-22k':  { min: 2500,   max: 18000,    field: 'priceUsdOunce' },
   'gold-18k':  { min: 2000,   max: 15000,    field: 'priceUsdOunce' },
   'silver':    { min: 15,     max: 500,       field: 'priceUsdOunce' },
   'platinum':  { min: 600,    max: 6000,      field: 'priceUsdOunce' },
